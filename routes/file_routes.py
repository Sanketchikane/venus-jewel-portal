...
import config  # must contain SECRET_SHARE_KEY
...
# ==========================================================
@file_bp.route("/api/delete-folder", methods=["POST"])
...
def api_share_folder():
    ...
    data = f"{folder_id}:{t}"
    secret_key = config.SECRET_SHARE_KEY.encode() if isinstance(config.SECRET_SHARE_KEY, str) else config.SECRET_SHARE_KEY
    s = hmac.new(secret_key, data.encode(), hashlib.sha256).hexdigest()

    url = f"/preview/folders-zip?ids={folder_id}&t={t}&s={s}"
    return jsonify({"link": request.url_root.rstrip("/") + url})


# ==========================================================
# PREVIEW MULTIPLE FOLDERS ZIP (SHARE LINK)
# ==========================================================
...
def preview_folders_zip():
    ...
    secret_key = config.SECRET_SHARE_KEY.encode() if isinstance(config.SECRET_SHARE_KEY, str) else config.SECRET_SHARE_KEY
    expected = hmac.new(secret_key, f"{ids}:{t}".encode(), hashlib.sha256).hexdigest()
    ...
    zip_buffer.seek(0)
    zipname = folder_names[0] + ".zip" if len(folder_names)==1 else f"Shared_{len(folder_names)}_Packets.zip"
