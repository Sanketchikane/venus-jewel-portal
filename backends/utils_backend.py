...
# initialize creds & clients once (fail fast if credential path is wrong)
...
def list_files_in_folder(folder_id, order="modifiedTime desc"):
    ...
    try:
        res = _drive_service.files().list(q=q, fields="files(id,name,mimeType,size,modifiedTime)", orderBy=order,
                                         includeItemsFromAllDrives=True, supportsAllDrives=True).execute()
        return res.get("files", [])
